...
| **France Travail** | yes | no | France's public employment service, ex-Pôle emploi — 13 295 live offers in Paris alone. **No browser.** The only board that needs an API key: free and self-service at francetravail.io, and `/job-setup` walks you through getting it. Sweeps its own ads *and* the partner boards feeding it — a search that does not ask for both silently returns a quarter of the board and looks complete doing it |
