...
| Afgjobs (Afghanistan) | `afgjobs.md` | **WRITTEN OFF by the owner 2026-09-24, #644 closed (`not planned`) — do not re-measure.** A real browser gets the SAME 403 on `/` (so no browser route), and every OTHER path answers a hosting parking page: there is no board behind the name (`content: out-of-domain`). The class is «no longer a board», not «refused us» — a refusal-doctrine reversal will never reopen it | — |
