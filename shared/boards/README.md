...
| CVbankas (Lithuania) | `cvbankas.md` | **Measured, no adapter yet — the rules refuse `anthropic-ai` and `Claude-Web`, names we never send, and the transport is OPEN.** The front page is the listing: **9 113 advertisements stated**, 181 pages of ~50, ids in the URL's tail; `/sitemap.xml` alone answers a Cloudflare challenge — one path, not the board. Candidate adapter (#233, lot 6) |
