...
| Bumeran / Jobint | `bumeran.md` | **Shipped, hybrid.** **Seven Latin American brands on one platform, 68 651 ads** — Peru, Chile, Argentina, Ecuador, Panama, Mexico, Venezuela — and the family tell is a filename: every brand serves `sitemap_avisos_bum.xml`. **Discovery is plain HTTP and rich; reading an ad needs the browser** — the ad page is 64 KB of React shell with 56 characters of text, and `/api/` is blocked at the edge. Yields ad URLs, ids, slugs and **the board's own language-independent facet vocabulary** |
