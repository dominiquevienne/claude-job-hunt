...
| HotNigerianJobs (Nigeria) | `hotnigerianjobs.md` | **Measured, no adapter yet — OPEN, and the unit is the post, not the position.** An index of 21 weekly sitemaps; week 37/2026 alone holds **4 190 `/hotjobs/<id>/` posts**, each a digest («Job Recruitment (124 Positions)»); the root lists 50, no stated total. Candidate adapter, unit named (#233, lot 7) |
| PNG JobSeek (Papua New Guinea) | `pngjobseek.md` | **Measured, no adapter yet — OPEN.** `/jobs` states **31 jobs** («Showing 1 – 25 of 31»), 25 `/jobs/<id>` links on page 1, the same 31 on the root. Small and served. Candidate adapter (#233, lot 7) |
| Undelucram.ro (Romania) | `undelucram.md` | **Measured, no adapter yet — OPEN.** `/ro/locuri-de-munca` states **1 459 results** over 146 pages of 10 (146 × 10 = 1 460), ids in the URL's tail; București 435, Timișoara 160 by the root's own city counts. Candidate adapter (#233, lot 7) |
...
| Tecnoempleo (Spain) | `tecnoempleo.md` | **Measured, no adapter yet — a hand-written file refusing six Anthropic names, `Claude-User` under `*`, and the transport is OPEN.** `/ofertas-trabajo/` states **2 592** IT offers, 30 a page («1-30 de 2.592»). The line «written by hand naming ClaudeBot» stays for the owner. Candidate adapter (#233, lot 8) |
| foundit Philippines (Philippines) | `foundit-ph.md` | **Measured, no adapter yet — the foundit Gulf stack: the same hand-written `ClaudeBot: Disallow /jobs/ /search/`, `Claude-User` under `*`, the sitemap index served** (37 children, `active-jobs-sitemap0/1`, `todays`). The root's «100,000+ Jobs» is a slogan. Candidate: `founditgulf.py` with this host as a second one (#233, lot 8) |
| Jobslin (Philippines and 20 countries) | `jobslin.md` | **Not a board — a country chooser, served, 0 advertisements.** The board is a sub-host per country: `ph.jobslin.com` served, 10 posts on its front page, no stated total, `Crawl-delay: 2`. Candidate: the sub-hosts (#233, lot 8) |
| The Ugandan Jobline (Uganda) | `theugandanjobline.md` | **Measured, no adapter yet — OPEN.** `/jobs-in-uganda` states **84 248 jobs** over 8 425 pages of 10 — an archive by the number; what marks a post live is the first question. The crawl-cost motive recorded on 2026-09-11 is not in the file read on 2026-09-13. Candidate adapter (#233, lot 8) |
| TopDev (Vietnam) | `topdev.md` | **Measured, no adapter yet — OPEN.** `/viec-lam/tim-kiem` states **4 942** IT jobs «[Update 13/9/2026]», 29 links a page, ids in the URL; the pager is the first question. Candidate adapter (#233, lot 8) |
...
| Barbados Job Register (Barbados) | `barbadosjobregister.md` | **Measured — no open path.** The register the Ministry of Labour points to: `www.` refuses its rules file (403 ×2, `x-cache: CONFIG_NOCACHE`) and the apex fails its TLS certificate (INDETERMINATE, not probed). Nothing past the rules was requested, no browser opened — the same shape as `kazibongo.md`, and the same doctrine question |
