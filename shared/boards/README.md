...
| Ghanajob.com (Ghana) | `ghanajob.md` | **Measured — browser route, no script.** AfricaWork family: managed template, 25-byte 403 to the declared client (twice), served to a tab with no challenge. The listing states **«262 Job ads found»** and its ten pages carry **237 distinct addresses — 25 short**, exactly one page's worth, unexplained; the adapter prints both. Id = the trailing number of the address; no JobPosting on the page |
