...
| Job Search Zambia | `jobsearchzm.md` | **Shipped.** **153 advertisements, every one of them dated in the sitemap** — so `--since` filters from the listing and opens no page: one request instead of 153. Its neighbour `gozambiajobs` carries no `<lastmod>` at all and refuses the same option. `--live` drops a *past* `validThrough` and keeps an *absent* one |
