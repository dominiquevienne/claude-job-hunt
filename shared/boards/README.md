...
| Cyprus Work (Cyprus) | `cypruswork.md` | **Reopened by the 2026-09-07 doctrine, and the transport is OPEN — measured, no adapter yet.** `ClaudeBot` named, `*` open — `identity()` says `claude-user`, `verdict()` sweeps since #230 — and the root, `/jobs/` and `/sitemap.xml` answer 200: **1 475 jobs stated by the listing, 1 475 distinct `/job/<id>/` URLs in the sitemap**, `JobPosting` JSON-LD on a job page. Candidate adapter (#233, lot 3) |
