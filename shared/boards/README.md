...
| Asako.mg (Madagascar) | `asako.md` | **Measured, no adapter — 251 offers behind a script on 2026-09-12, and a name the .mg registry no longer delegates on 2026-09-13** (NXDOMAIN at `ns.nic.mg` and on 1.1.1.1 / 8.8.8.8 / 9.9.9.9, twenty-five hours after a 200 on Vercel). No adapter can be built, by script or browser, against a name that does not resolve; a reading to repeat (`dig @ns.nic.mg asako.mg NS`), not a verdict. Candidate adapter (#233, lot 7) when the delegation returns |
