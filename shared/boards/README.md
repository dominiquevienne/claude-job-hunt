...
| Softy | `softy.md` | **Shipped — browser only, by choice.** The third French SME/ETI ATS, after Taleez and Flatchr. Its robots.txt allows `*` everything and then **disallows the AI agents by name, Anthropic's twice** — so the sweep runs in the user's own browser session rather than as a script. An ad can span seven towns and the page shows one: the rest live in a tooltip that only a **real hover** opens |
...
user's own browser session under their own account; for those, dormancy expiry means
...
`jobup`'s browser prerequisite cost every browser-less user their Swiss
...
`indeed.md`, `cadremploi.md` and `softy.md` each use the user's own browser session
