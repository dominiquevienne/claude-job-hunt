...
| Wazifaha (Afghanistan) | `wazifaha.md` | **Shipped 2026-09-24 (#642): Afghanistan's first generalist** (ACBAR is the humanitarian market). **272 adverts against the 272 the list states**, 16 pages, the last carrying exactly the 2 the arithmetic predicted. **The board CLAMPS past its last page** — `?page=17` re-serves page 16, so a walk stopping on an empty page never terminates; the opposite of the Iranian pager that crushes (#894 family, two new shapes in two days). Counted by container, not by link (18 cards, 48 addresses on a page); the employer is printed twice with two truncations and a positional read slid the province on 24 of 272 rows. 177 of 272 cards print a gender, never carried. | `wazifaha.py` |
| Bast.af (Afghanistan) | `bast-af.md` | **Measured 2026-09-17 then 2026-09-24 (#643), no script yet: the root is still the same 4 871-byte shell to the byte, `Crawl-delay: 1` recovered, and the sitemap's 14 URLs are static pages with not one advert.** The bundle names `db.bast.af/api` as its axios base but **the list endpoint is assembled from variables and is not recoverable statically** — naming one and trying it would be composing a path the site has not shown. Next reading: a browser watching its own requests. Open work, no refusal. | — |
| Afgjobs (Afghanistan) | `afgjobs.md` | **Re-measured 2026-09-24 (#644): the tab was connected, and a real browser gets the SAME 403 on `/` — so no browser route. But every OTHER path answers a hosting parking page («This Page Does Not Exist», with an advert): there is no board behind the name (`content: out-of-domain`).** The 2026-09-17 «the rules file is served» is corrected: the rules path is one of those 404s | — |
...
| HireLebanese (Lebanon) | `hirelebanese.md` | **Shipped 2026-09-23 (#657): Lebanon's first adapter.** **3 232 adverts against the 3 232 the board states, and the 41 location counts it states are the 41 the rows give** — 65 pages of a plain GET (`searchresults.aspx?…&pg=N`; the POST and ViewState belong to the search form, not the browse route). Every page states its window, so the walk is checked at each page — which is how a featured advert's star image was found dropping one panel on eighteen pages (3 199 against 3 232), and how, once the total agreed exactly, the place-by-place count still exposed labels truncated at 20 characters. 750 employers, filed 24.06 → 23.09, 33 featured, 0 contact leaks on 3 232. | `hirelebanese.py` |
...
| Zangia (Mongolia) | `zangia.md` | **Livré 2026-09-23 (#660) :** le généraliste mongol — **7 945 annonces émises contre les 7 945 qu'il énonce**, quatre-vingt-dix par page ; `/job/list` est une **coquille Next.js** (47 512 o, aucune carte, aucun compte) et les annonces viennent du JSON que son propre paquet appelle, `new-api.zangia.mn/api/jobs/search` — trouvé dans les 54 chunks (`axios baseURL`), **pas** sur les deux hôtes que la page `preconnect` ; **au-delà de la dernière page l'enveloppe ré-énonce son total à ZÉRO** (`{"total": 0, "page": 0, "limit": 0, "totalPages": 0}`), donc un témoin relu à chaque tour finit à zéro et accuse une marche exacte de sur-lire — le total se lit page une et **jamais plus** ; **second témoin, une PARTITION** : les dix niveaux du board somment à 93+3 127+454+2 676+255+938+171+128+66+37 = **7 945**, le total énoncé exactement — chaque annonce porte un niveau et un seul, et le total s'accorde à une décomposition calculée à part (`levels`) ; **le téléphone du recruteur voyage dans `contact` sur 84 des 90 lignes de la page une** — jeté ENTIER, jamais masqué en gabarit ; `lat`/`lng` jamais lus ; les critères sur la PERSONNE (`age_requires` sur 2 605, `for_hbi`, `is_retired`) ne sont pas propagés (#183) et se redemandent par facette (`--type 45plus` 412, `disability` 11) ; une clé ABSENTE de la route détail va dans `criteria_unknown` — #885 retourné : une liste vide y affirmerait une connaissance que personne n'a ; une coupure du transport au 49e des 89 appels a jeté 48 pages, donc les lignes sortent **au fil de l'eau** et une marche coupée les garde en se déclarant courte ; et un hash de chunk lu dans la copie de la veille rend **404** — le site a redéployé, ce n'est pas un refus | **`zangia.py jobs [--pages N\|--all] [--type T] [--query Q]` · `ad --url\|--code` · `types` · `levels`** |
...
| Melli Kar (Iran) | `mellikar.md` | **Livré 2026-09-23 (#631) :** board ASP.NET WebForms marché par POSTBACK — 18 annonces par tour, `/jobs` rend 404, les routes sont `/jobs/category/<slug>` et `/jobs/city/<slug>` ; **la liste SE RÉINITIALISE à sa première tranche** (deux fois sur huit tours, mesuré), donc «&nbsp;un tour sans nouveauté&nbsp;» n'est PAS la fin — cette règle y émettrait 18 sur 5 365, sortie 0, sans erreur ; et «&nbsp;sans nouveauté&nbsp;» est ambigu de TROIS façons (réinitialisation · le serveur qui REMONTE ce qu'on a déjà vu · la vraie fin), que seul **le compte de lignes de la PAGE** sépare : il croît, retombe à 18, cesse de croître ; le postback porte les trois champs d'état **et rien d'autre — renvoyer tout le formulaire rend 500** ; la page ACCUMULE (111 ko → 424 ko en quatre tours), donc la marche est **quadratique en octets** et bornée par défaut ; témoin = les huit comptes par catégorie de l'ACCUEIL, **qui bougent à chaque lecture** (5316→5358→5365) donc calculés — le «&nbsp;بیش از 70000&nbsp;» est «&nbsp;plus de 70 000&nbsp;», une affirmation ; `lblEnteshar` («&nbsp;publication&nbsp;») porte le CONTRAT ; salaire derrière une authentification, **déclaré** ; une annonce n'a **aucune adresse propre** (postback), donc `ad` refuse en le disant | **`mellikar.py categories` · `jobs --category <slug> [--rounds N\|--all]`** |
| E-Estekhdam (Iran) | `e-estekhdam.md` | **Livré 2026-09-24 (#632) :** l'agrégateur iranien derrière une **coquille Nuxt** de 8 383 o titrée «بارگذاری» — la liste est `POST /search-api/search?page=N` (corps JSON de filtres, 20 par page) trouvée dans le paquet (`axios baseURL`), et **elle répond 201, pas 200** : un contrôle écrit pour `== 200` refuse chaque page ; **LE PAGER ÉCRASE AU LIEU DE FINIR** — pages 100, 500 et 5000 identiques À L'OCTET (`md5 973706fe9df1`), `ok: true`, sans erreur, alors qu'une requête étroite finit proprement sur `data: {}` — **donc les deux règles d'arrêt usuelles échouent en silence** (« page vide » ne s'arrête jamais sur une requête large, « rien de nouveau » s'arrête à 2 000 et présente le plafond comme le board) : on s'arrête sur une page qui **RÉPÈTE** la précédente et on le DIT ; **aucun témoin n'existe** — `meta` sans total, `filter-options` 253 métiers / 50 provinces / 133 technologies **sans un seul compte**, `/sitemap.xml` 404 — donc **chaque compte porte « borne inférieure » dans la même phrase**, et la seule borne qui existe est le plafond de page ; `data` est une **liste quand pleine et un objet `{}` quand vide** ; la partition par provinces n'en est pas une (une annonce nationale en porte 31 → dédoublonnage par `id`) ; **`sort`, `posted` et les deux facettes nommées sont refusés PAR LEUR NOM dans `--filter`** — et le refus a dû être rendu ATTEIGNABLE pour en être un ; cadence **5 s** bien que le groupe `*` n'impose rien, parce que le fichier en pose 5 à tout robot qu'il nomme ; le genre imprimé sur chaque annonce n'est jamais porté (#183) ; **l'hôte ne tient pas une marche de cent requêtes** — coupé deux fois en deux essais (SSL EOF, puis délai dépassé), d'où un essai de reprise, les lignes au fil de l'eau et trois fins distinguées | **`eestekhdam.py jobs [--pages N\|--all] [--where KEY] [--query Q] [--filter KEY=VALUE]` · `filters`** |
...
| MyJobs (Myanmar) | `myjobs-mm.md` | **Shipped 2026-09-23 (#636): Myanmar's second living board.** **177 adverts against the 177 its thirty category counts sum to**, over the 15 pages its pager bounds, the last carrying exactly the **9** the arithmetic predicted — three separate statements agreeing, and the prediction was tested before a line was written. 30 of 30 per-category counts agree. Salary «Hidden» on 30 and «Negotiable» on 32 — declared, not dropped. *And the nine-digit telephone rule had to be taken off the salary field: a Myanmar range is twelve digits and the rule destroyed 113 of 115 before the guard caught it — right about the shape, wrong about the field.* | `myjobsmm.py` |
