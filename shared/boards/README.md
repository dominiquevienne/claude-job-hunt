...
| Albania Jobs (Albania) | `albaniajobs.md` | **Measured, no adapter yet — the rules refuse `anthropic-ai`, a name we never send, and the transport is OPEN.** WordPress board, `Crawl-delay: 10` honoured; `job_listing-sitemap.xml` holds **123 advertisements** with real `<lastmod>` (59 distinct), the newest 2026-08-17; the listing page renders by script. Candidate adapter (#233, lot 4) |
| foundit Gulf (Gulf + Egypt) | `founditgulf.md` | **Measured, no adapter yet — a hand-written `ClaudeBot: Disallow /jobs/ /search/`, `Claude-User` under `*`, and the transport is OPEN.** Three active-jobs sitemaps, **58 930 distinct advertisements** — AE 29 360, SA 13 600, **EG 12 080**, QA 2 215, KW 777, BH 263, OM 32 by the slug; `<lastmod>` is a rebuild stamped by the second; today's sitemap 1 041, all among them. Candidate adapter (#233, lot 4) |
