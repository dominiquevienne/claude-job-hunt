...
| Ango Emprego (Angola) | `angoemprego.md` | **Shipped.** **The smallest archive of the three Angolan boards and the largest flow** — 1 434 held against `jobartis`'s 38 547, and 924 published since 1 August against its 202. **Two blocks with six years of nothing between**: 349 from a 2020 launch and 1 085 from 2026-07-29 onward, so `--since` is what makes it usable. The busiest-day check fired at 15.8 % and reading the distribution said what it was. **Its dates were suspected of being regeneration stamps and are not** — `lastmod` equals `datePosted` on 4 of 4. Two named job sitemaps, never a `job_listing*` wildcard: the taxonomies match it |
