...
| XpressJobs (Sri Lanka) | `xpressjobs.md` | **Not shipped — open, moved, and rendered client-side.** The country page named `xpressjobs.lk`; **the guard reported the rules as read from `xpress.jobs`**, so the board has changed domain. Its rules are 70 bytes with an empty `Disallow:` — as open as a file gets — and its sitemap holds **8 815 URLs and zero advertisements**: 8 725 are `/Organization/` employer profiles, none carries a `lastmod`. **Every route returns the same 1 766-byte React shell**, checked for `data-page`, `__NEXT_DATA__` and inline JSON — none present — and the API base is computed at runtime (`Na()+"api/"`). *Not refused, not absent, not a challenge: the object is not on the wire* |
