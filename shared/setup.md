...
| Cegid Talentsoft | The employers they would work for, **as careers host labels** — `businessfrance-recrute`, `groupeadp-recrute`. **No login, no browser, no key.** Ministries, airports, energy, large agencies. No directory exists, so ask for the URL; a wrong label does not 404, it fails to resolve. Worth saying: the listing already carries the address and the date, so a scan is useful without opening a single ad |
